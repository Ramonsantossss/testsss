try {
  ...
  ///////////////////////////////////////////////
  const imagemdomenu = fs.readFileSync('./clover2.jpg')
  ...
  `,
        ...
        */
        ...
        function adicionarGrupo(id) {
          ...
          const dataTermino = moment().add(0, 'days').format('YYYY-MM-DD');
        ...
        // Função para verificar e remover grupos com período de aluguel expirado
        function verificarData() {
          ...
          for (const id in grupos) {
            ...
            if (hoje.isSameOrAfter(dataTermino, 'day')) {
              console.log(`Removendo o bot do grupo ${id} devido ao período de aluguel expirado`);
        ...
        // Exemplo de uso para verificar e remover grupos expirados
        //verificarEAtualizarGrupos();
        ...
        const usuarioRegistradoIndex = registros.findIndex((usuario) => usuario.id === sender);


        function adicionarSaldo(jid) {
          try {
            // Encontrar o usuário com base no JID
            const usuarioIndex = registros.findIndex((usuario) => usuario.id === jid);

            if (usuarioIndex !== -1) {
              // Se o usuário for encontrado, aumenta o saldo em 1
              registros[usuarioIndex].saldo += 1;

              // Salva os registros no arquivo
              fs.writeFileSync("./functions/registros.json", JSON.stringify(registros));
            ...
            console.log(e);
            return false; // Erro ao processar a operação
          }
        }
        ...
        switch (command) {
          ...
          case "login":
            if (usuarioRegistradoIndex !== -1) {
              // Se o usuário já estiver registrado, envia uma mensagem informando
              return enviar(resposta.norg);
            }
            
            try {
              // Adiciona um novo registro com saldo 0
              registros.push({
                id: sender,
                nome: pushname,
                horario: hora,
            ...
            enviar(`[⚙] Registrado com sucesso 
              ...
              🎉🎈 Parabéns por se registrar! Mande *menu e veja todos os comandos...`);
            break;
          ...
          case "alugar": {
            ...
            if (usuarioRegistradoIndex) {
              // Se o usuário já estiver registrado, envia uma mensagem informando
              return enviar(resposta.registro);
            }
            const mercadopago = require('mercadopago');
            const string = args.join(' ')
            const mainn = string.split('app.com/')[1]

            const accessToken = "APP_USR-8259792445335336-080911-dea2c74872b688a02354a83a497effba-1445374797";

            mercadopago.configure({
              access_token: accessToken
            });
            client.sendMessage(sender, { text: `✅ Pagamento PIX gerado com sucesso!\n💳 Total: R$15\n📆 Pague  em até: 10 minutos \n\nCaso vc nao foi notificado pelo bot sobre o pagamento ou teve algum problema mande\n/ suporte e aguarde o adm entrar em contato com vc...` });
            client.sendMessage(from, { text: `✅ Pagamento PIX gerado com sucesso!\n💳 Total: R$15\n📆 Pague  em até: 10 minutos \n\nCaso vc nao foi notificado pelo bot sobre o pagamento ou teve algum problema mande\n/ suporte e aguarde o adm entrar em contato com vc...` });
            const amount = 0.01;

            const generatePixPayment = async (amount) => {
              if (!amount) {
                reply('Por favor, insira um valor para gerar um pagamento.');
                return;
              }

              const email = 'rs7569618@gmail.com'; // Qualquer email

              const payment_data = {
                transaction_amount: 0.01,
                description: 'Pagamentos Via CLI',
              ...
              client.sendMessage(sender, { text: `${data.body.point_of_interaction.transaction_data.qr_code}` })

              const paymentId = data.body.id;
              console.log(paymentId)
              // Define um tempo limite (em milissegundos) para o pagamento ser confirmado
              ...
              while (!isPaymentConfirmed && Date.now() - startTime < timeout) {
                ...
                if (pagamentoStatus === 'approved') {
                  client.sendMessage(sender, { text: `✅ Pagamento aprovado com sucesso!` })
                  adicionarSaldo(sender);
                  client.groupAcceptInvite(`${mainn}`)
                  isPaymentConfirmed = true;
                } else {
                  console.log('Aguardando pagamento...');
                  await new Promise(resolve => setTimeout(resolve, 30000)); // Verificar a cada 30 segundos
                }
              }

              if (!isPaymentConfirmed) {
                reply('❗ Tempo de pagamento expirado ou pagamento não confirmado.');
              }
            };
          ...
          case 'ytmp4':
            try {
              ...
              var url = q;
              var anikit = await fetchJson(`https://dark-lime-snapper-hat.cyclic.app/anikit/ytmp4?username=SUPREMO&key=Adm&videoUrl=${url}`)
              client.sendMessage(from,
                {
          ...
          case 'ytmp3':
            try {
              ...
              client.sendMessage(from, { react: { text: '🕚', key: info.key } })
              var url = q;
              var anikit = await fetchJson(`https://dark-lime-snapper-hat.cyclic.app/anikit/ytmp3?username=SUPREMO&key=Adm&videoUrl=${url}`)
              client.sendMessage(from,
                {
          ...
          case 'play_video':
            //case 'ytmp4':
            try {
              if (!q) return reply(`˖⸙̭❛✰❤️Coloque o nome da musica para o bot baixar`)
              client.sendMessage(from, { react: { text: '🕚', key: info.key } })
              var url = q;
              var anikit = await fetchJson(`https://dark-lime-snapper-hat.cyclic.app/anikit/playmp4?username=SUPREMO&key=Adm&query=${url}`)
              client.sendMessage(from,
                {
                  video: { url: anikit.url },
                  mimetype: 'video/mp4'
  ...
  `
    ...
    ╚═════ °❀•°💜°•❀°═════╝`)
          ...
          case "tiktokvd": {
            ...
            reply("aguarde...")
            const fitat = await fetchJson(`https://dark-lime-snapper-hat.cyclic.app/anikit/tiktok?username=SUPREMO&key=Adm&videoUrl=${q}`)
